...
func (lca *LCA) init(g [][]int) {
	...
	lca.ancestors = make([][]int, lca.n)
	for i := 0; i < lca.n; i++ {
		lca.ancestors[i] = make([]int, lca.x)
	}

	// depthとparentの初期化
	parent := make([]int, lca.n)
...
func calc() {
	...
	for i := 0; i < Q; i++ {
		...
		out.Println(lca.distance(a, b) + 1)
	}
}
