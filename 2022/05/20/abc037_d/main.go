...
func solve(i, j int) int {
	...
	isOut := func(x, y int) bool {
		return x < 0 || x >= len(field) || y < 0 || y >= len(field[0])
	}
	dx := []int{+1, -1, +0, +0}
	dy := []int{+0, +0, +1, -1}
	...
	for t := 0; t < 4; t++ {
		...
		if isOut(x, y) || field[x][y] <= field[i][j] {
			continue
		}
		ans = mod.add(ans, solve(x, y))
	}
