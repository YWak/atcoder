...
func NewWaveletMatrix(values []int) *WaveletMatrix {
	...
	return &WaveletMatrix{index, zeros, sum, start, bitsize}
}
...
// RangeFreqは区間[l, r)に含まれる要素のうち[lower, upper)である要素の数を返します。
func (w *WaveletMatrix) RangeFreq(l, r, lower, upper int) int {
	return 0
}

// PrevValueは区間[l, r)に含まれる要素のうちupperの次に小さいものを返します。
func (w *WaveletMatrix) PrevValue(l, r, upper int) int {
	return 0
}

// NextValueは区間[l, r)に含まれる要素のうちlowerの次に大きいものを返します。
func (w *WaveletMatrix) NextValue(l, r, lower int) int {
	return 0
}
