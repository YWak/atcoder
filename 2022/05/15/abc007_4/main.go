...
func solve(a int) int {
	...
	// dp[i][j]はi桁目まで見て、aをj(0:超えているかどうか不明, 1:超えていないことが確定)なときの場合の数
	dp := make([][]int, n+1)
	for i := 0; i < len(dp); i++ {
		dp[i] = make([]int, 2)
	}
	dp[0][0] = 1
	m := []int{1, 2, 3, 4, 4, 5, 6, 7, 8, 8}
	for i := 0; i < n; i++ {
		// 前の桁までで数値を超えていないことが確定しているケース
		// もう超えていないことがわかっていればどの数を使ってもよい
		dp[i+1][1] = dp[i][1] * m[9]
		// 未確定の場合でも、桁の数より小さい値を採用すれば小さくなる
		if d[i] > 0 {
			dp[i+1][1] += dp[i][0] * m[d[i]-1]
		}

		// 確定しない場合は見ている桁の数しか使えない
		// 4と9は使えないので以降0になる
		if d[i] == 4 || d[i] == 9 {
			dp[i+1][0] = 0
		} else {
...
func calc() {
	...
	out.Println(b - a + 1 - (solve(b) - solve(a-1)))
}
