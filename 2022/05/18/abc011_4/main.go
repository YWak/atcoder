...
func calc() float64 {
	...
	cx, cy := abs(x/D), abs(y/D)
	minstep := cx + cy
	if minstep > n || (n-minstep)%2 == 1 {
		return 0.0
	}

	ans := 0.0
	// sum(n! / (a! b! c! d!))
	for dx := 0; dx <= n-minstep; dx += 2 {
		divided_by_4 := 0 // 4で割った回数
		dy := n - minstep - dx

		a := []int{cx + dx/2, cy + dy/2, dx / 2, dy / 2}
