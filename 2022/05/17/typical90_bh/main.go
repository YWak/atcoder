...
func calc() {
	...
	// 前後から見たとき、最後の値がkであるLISの長さの最大値を管理する
	M := 300000 + 1
	front := NewRangeMaxQuery()
	front.init(M)
	back := NewRangeMaxQuery()
	back.init(M)

	// i番目の値を適用する前のbackの値を持っておく
	backhist := make([]int, n)

	for i := n - 1; i >= 0; i-- {
		m := back.query(1, a[i])
		v := back.get(a[i])
		back.update(a[i], max(m+1, v))
		backhist[i] = v
	}

	ans := 0
	for i := 0; i < n; i++ {
		// 前から見てa[i]未満で作られるLISの長さの最大値 + 後ろから見てa[i]以下で作られるLISの長さの最大値
		f := front.query(1, a[i])
		b := back.query(1, a[i]+1)
		ans = max(ans, f+b)
		front.update(a[i], max(front.query(1, a[i])+1, front.get(a[i])))
		back.update(a[i], backhist[i])
	}
	out.Println(ans)
}
