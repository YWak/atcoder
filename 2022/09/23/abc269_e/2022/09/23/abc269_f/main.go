...
var mod = Mod998244353()

func calc() {
	_, m := in.NextInt2()
	Q := in.NextInt()
	// 四隅が白
	f2 := func(a, b, c, d int) int {
		x := divceil(b-a+1, 2)
		y := divceil(d-c+1, 2)

		s := mod.add(mod.mul(a-1, m), c)
		l := mod.mul(x, s)
		mod.chadd(&l, mod.mul(mod.mul(x, x-1), m))
		mod.chmul(&l, y)

		r := mod.mul(x, mod.mul(y, y-1))
		return mod.add(l, r)
	}

	for i := 0; i < Q; i++ {
		a, b, c, d := in.NextInt4()
		a1, a2, b1, b2, c1, c2, d1, d2 := a, a, b, b, c, c, d, d
		if (a+c)%2 == 0 {
			a2, c2 = a+1, c+1
		} else {
			a1 = a + 1
		...
		ans1 := f2(a1, b1, c1, d1)
		ans2 := f2(a2, b2, c2, d2)
		// debug(":1:", a1, b1, c1, d1, ":2:", a2, b2, c2, d2, ans1, ans2)
		out.Println(mod.add(ans1, ans2))
	}
}
