...
func calc() {
	n := in.NextInt()
	// にぶたん
	query := func(c1, c2, t1, t2 int, call func(a, b, c, d int)) int {
		for t1 != t2 {
			m := (t1 + t2) / 2
			expected := m - t1 + 1
			call(c1, c2, t1, m)
			actual := in.NextInt()
			debug("ex", expected, "ac", actual)
			if expected == actual {
				t1 = m + 1
			} else {
				t2 = m
	...
	y := query(1, n, 1, n, func(a, b, c, d int) { out.Printf("? %d %d %d %d\n", a, b, c, d) })
	x := query(1, n, 1, n, func(a, b, c, d int) { out.Printf("? %d %d %d %d\n", c, d, a, b) })
