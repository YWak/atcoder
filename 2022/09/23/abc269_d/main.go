...
var black = NewIntInt(2001, 2001, 0)
var used = NewIntInt(2001, 2001, 0)
...
func calc() {
	...
	for i := range black {
		for j, b := range black[i] {
			if b != 1 || used[i][j] != 0 {
				continue
			}
			ans++
			queue := []*point{{i - 1000, j - 1000}}

			for len(queue) > 0 {
				p := queue[0]
				used[p.x+1000][p.y+1000] = 1
				queue = queue[1:]
				for t := 0; t < 6; t++ {
					x := p.x + dx[t]
					y := p.y + dy[t]

					if x < -1000 || y < -1000 || x > 1000 || y > 1000 || black[x+1000][y+1000] == 0 || used[x+1000][y+1000] == 1 {
						continue
					}
