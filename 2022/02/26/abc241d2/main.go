...
func calc() {
	...
	for q := 0; q < Q; q++ {
		...
		z.add(x)
		if c == 1 {
			queries[q] = &Query{c, x, -1}
		} else {
			k := in.NextInt()
			queries[q] = &Query{c, x, k}
		}
	}
	z.update(0)
	bit := make(BinaryIndexedTree, z.Len()+1)
	for _, q := range queries {
		if q.c == 1 {
			bit.add(z.indexOf(q.x), 1)
		} else {
			x := z.indexOf(q.x)
			if q.c == 2 {
				ok, ng := -1, x+1
				...
				if ok == -1 {
					...
					out.Println(z.valueOf(ok))
				}
			} else {
				ok, ng := z.Len(), x-1
				for abs(ok-ng) > 1 {
					m := (ok + ng) / 2
					s := bit.sum(m) - bit.sum(x-1)
					if s >= q.k {
						ok = m
					} else {
						ng = m
...
func NewZip() *zip {
	return &zip{map[int]int{}, map[int]int{}}
}

func (z *zip) add(x int) {
	z.v2i[x]++
}

func (z *zip) update(offset int) {
	keys := make([]int, 0, len(z.v2i))
	for k := range z.v2i {
		keys = append(keys, k)
	}
...
func (z *zip) indexOf(value int) int {
	return z.v2i[value]
}

func (z *zip) valueOf(index int) int {
	return z.i2v[index]
}
