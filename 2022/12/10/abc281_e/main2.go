...
func NewTreap(comparator func(a, b int) int) *Treap {
	return &Treap{nil, comparator}
}

// NewIntTreap は、int型のキーを使用し、昇順に保存するTreapを作成して返します。
func NewIntTreap() *Treap {
	return NewTreap(func(a, b int) int {
		if a == b {
			return 0
		}
		if a < b {
...
// Get は、keyに対応する値を探して返します。 存在しない場合は0を返します。
...
func (t *Treap) GetKth(k int) (int, int, bool) {
	if k < 0 || t.Len() < k {
		return 0, 0, false
	}
...
// 対応するキーがなければnilを返します。
func (t *Treap) Find(key int) (int, bool) {
	n := t.root
	var ans *node
...
func (t *Treap) Put(key, value int) int {
	n, v, _ := t._put(t.root, key, value, rand.Intn(1<<60))
	t.root = n
	t._update(t.root)
...
// Addはkeyに対応する値にvalueを加えた値を返します。
// keyに対応する値がなければ0とします。
func (t *Treap) Add(key, value int) int {
	v, _ := t.Get(key)
	t.Put(key, v+value)
	return v + value
}

func (t *Treap) Sub(key, value int) int {
	return t.Add(key, -value)
}

func (t *Treap) SubAndRemoveZero(key, value int) int {
	v, _ := t.Get(key)
	v -= value
	if v == 0 {
		t.Remove(key)
...
func (t *Treap) Increment(key int) int {
	return t.Add(key, 1)
}

func (t *Treap) Decrement(key int) int {
	return t.Add(key, -1)
}

func (t *Treap) DecrementAndRemoveZero(key int) int {
	return t.SubAndRemoveZero(key, 1)
}

func (t *Treap) _put(n *node, key, value int, pri int) (*node, int, bool) {
	if n == nil {
		return &node{key, value, pri, 1, nil, nil}, 0, false
	}
	c := t.comparator(key, n.key)
	if c == 0 {
		v := n.value
	...
	if c <= 0 {
		nn, v, b := t._put(n.left, key, value, pri)
		n.left = nn
		if n.left.pri < n.pri {
			n = t._rotatel(n)
		}

		return t._update(n), v, b
	} else {
		nn, v, b := t._put(n.right, key, value, pri)
		n.right = nn
...
func (t *Treap) Remove(key int) int {
	n, v := t._remove(t.root, key)
	t.root = t._update(n)
	return v
}

func (t *Treap) _remove(n *node, key int) (*node, int) {
	if n == nil {
		return nil, 0
	}
	c := t.comparator(key, n.key)
...
func (n *node) entry() [][]int {
	ans := [][]int{}
	if n.left != nil {
		ans = append(ans, n.left.entry()...)
	}
	ans = append(ans, []int{n.key, n.value})
	if n.right != nil {
		ans = append(ans, n.right.entry()...)
	}
...
func calc() {
	...
	for i := 0; i < n-m+1; i++ {
		...
		if _, e := used.Get(r); e {
			// 使用中なら合計から削除
			size--
			used.DecrementAndRemoveZero(r)
			s -= r
		} else {
			// 未使用なら候補から削除
			cand.DecrementAndRemoveZero(r)
		}
		if size > 0 {
			// 最大値を候補に入れる
			p, _, _ := used.GetKth(1)
		...
		for j := size; j < k; j++ {
			...
			cand.DecrementAndRemoveZero(p)
			used.Increment(p)
			s += p
			size++
		}

		// debug(cand.entry(), used.entry())
	}
