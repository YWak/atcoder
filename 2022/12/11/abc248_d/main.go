...
type Dictionary struct {
	...
	// rankはkビット目までの1の数を返します。
	rank1 func(k int) int

	rank0 func(k int) int

	// nextはk個目の1のビットの次の位置を返します。
	next func(k int) int
}

func NewDictionary(bs []int) Dictionary {
	n := len(bs) + 1
	bitsize := 64

	// bit列として保存する
	all := make([]uint64, divceil(n, bitsize))
	// 大ブロック. bignごとにbit数の合計を保存する
	big := make([]uint32, divceil(n, bitsize)+1)

	get := func(k int) int {
		return int(all[k/bitsize]>>(k%bitsize)) & 1
	}
	set := func(k, b int) {
		all[k/bitsize] += uint64(b << (k % bitsize))
	}
	...
	for i, b := range all {
		big[i+1] = big[i] + uint32(bits.OnesCount64(b))
	}

	// kビット目までに1がいくつあるか
	rank1 := func(k int) int {
		mask := uint64((1 << (k % bitsize)) - 1)
		return int(big[k/bitsize]) + bits.OnesCount64(all[k/bitsize]&mask)
	}
	rank0 := func(k int) int {
		return k - rank1(k)
	}
	...
	return Dictionary{get, rank1, rank0, next}
}
