...
func calc() {
	...
	// [l, r)で sum(l, m) < sum(m, r)となる最大のmを返す
	mid := func(l, r int) int {
		ok, ng := l, r
		for abs(ok-ng) > 1 {
			m := (ok + ng) / 2
			s1 := sum(l, m)
	...
	for b := 0; b < n; b++ {
		...
		pat1 := [][]int{{sum(0, a), sum(a, b)}}
		if a+1 <= b {
			pat1 = append(pat1, []int{sum(0, a+1), sum(a+1, b)})
		}

		pat2 := [][]int{{sum(b, c), sum(c, n)}}
		if c+1 <= n {
			pat2 = append(pat2, []int{sum(b, c+1), sum(c+1, n)})
		}
		for _, p1 := range pat1 {
			for _, p2 := range pat2 {
				chmin(&ans, max4(p1[0], p1[1], p2[0], p2[1])-min4(p1[0], p1[1], p2[0], p2[1]))
			}
		}
	}
