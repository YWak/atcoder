...
func calc() {
	n, m := in.NextInt2()
	s, t := in.NextInt2d(-1, -1)
	gen := NewGraph(n)
	gsn := NewGraph(n)
	for i := 0; i < m; i++ {
		u, v := in.NextInt2d(-1, -1)
		a, b := in.NextInt2()
		gen.AddWeightedEdge(u, v, a)
		gen.AddWeightedEdge(v, u, a)
		gsn.AddWeightedEdge(u, v, b)
		gsn.AddWeightedEdge(v, u, b)
	}
	yen := gen.DijkstraAll(s)
	snk := gsn.DijkstraAll(t)
	d := make([]int, n)
	for i := 0; i < n; i++ {
		d[i] = yen[i] + snk[i]
	}
