...
func calc() {
	...
	for i := 0; i < m; i++ {
		...
		g.AddWeightedEdge(a, b, c, i+1)
		g.AddWeightedEdge(b, a, c, i+1)
	}
	ans := g.DijkstraAll(0)
...
func (g *Graph) AddEdge(s, t int) {
	g.AddWeightedEdge(s, t, 1, -1)
}

// AddWeightedEdge は重み付きの辺を追加します。
func (g *Graph) AddWeightedEdge(s, t, w, i int) {
	g.list[s] = append(g.list[s], Edge{t, w, i})
}
