...
func calc() {
	...
	// currのノードを(黒, 白)に塗るパターンの数
	var dfs func(curr, prev int) (int, int)
	dfs = func(curr, prev int) (int, int) {
		// 末端
		if len(g[curr]) == 1 && g[curr][0] == prev {
			return 1, 1
		}

		// それ以外は全部白の場合と全パターンを計算する
		all := 1
		white := 1
		for _, next := range g[curr] {
			...
			b, w := dfs(next, curr)
			mod.chmul(&all, mod.add(b, w))
			mod.chmul(&white, w)
		}

		// 全部白の場合は黒にできる。どの場合でも白にできる
		return white, all
	}

	black, white := dfs(0, -1)
	out.Println(mod.add(black, white))
}
