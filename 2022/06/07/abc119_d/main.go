...
func calc() {
	...
	t := []int{-INF18}
	for i := 0; i < a; i++ {
		s = append(s, in.NextInt())
	}
	for i := 0; i < b; i++ {
		t = append(t, in.NextInt())
	}
	s = append(s, INF18)
	t = append(t, INF18)

	upper := func(arr []int, v int) int {
		ok, ng := len(arr), -1
		for abs(ok-ng) > 1 {
			mid := (ok + ng) / 2
			if arr[mid] > v {
	...
	for q := 0; q < Q; q++ {
		...
		x := in.NextInt()
		ans := INF18
		s0, s1 := lower(s, x), upper(s, x)
		t0, t1 := lower(t, x), upper(t, x)

		chmin(&ans, x-min(s0, t0))
		chmin(&ans, max(s1, t1)-x)
		chmin(&ans, x-s0+t1-s0)
		chmin(&ans, x-t0+s1-t0)
		chmin(&ans, s1-x+s1-t0)
		chmin(&ans, t1-x+t1-s0)
		out.Println(ans)
	}
}
