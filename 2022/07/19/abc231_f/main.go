...
func calc() {
	...
	items := []*item{{-1, -1, 1, 0, 0}}
	for _, v := range all {
		last := items[len(items)-1]
		if last.a == v.a && last.b == v.b {
			last.c++
		} else {
	...
	// i番目のプレゼントを高橋くんに, j番目のプレゼントを青木くんに渡すとする
	// a[j] <= a[i] かつ b[i] <= b[j]となる
	ans := 0
	bit := make(BinaryIndexedTree, len(c)+10)

	for _, it := range items {
		s := bit.Sum(len(c)-1) - bit.Sum(it.bb-1)
		// debug(it.a, it.b, it.c, s)
		ans += s * it.c
		ans += it.c * it.c
