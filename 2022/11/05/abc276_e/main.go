...
func ng(x, y int) bool {
	if x == -1 || y == -1 || x == h || y == w || c[x][y] == '#' {
		return true
	}
	return false
}
func bfs(x, y int) bool {
	...
	for len(queue) > 0 {
		...
		for i := 0; i < 4; i++ {
			...
			queue = append(queue, &point{xx, yy})
			used[xx][yy] = 1
		}
	}
...
func calc() {
	...
	for i := 0; i < 4; i++ {
		for j := i + 1; j < 4; j++ {
			x1, y1 := sx+dx[i], sy+dy[i]
			x2, y2 := sx+dx[j], sy+dy[j]

			if ng(x1, y1) || ng(x2, y2) {
				continue
			}
			c1, c2 := c[x1][y1], c[x2][y2]
			c[x1][y1] = 's'
			c[x2][y2] = 'e'
			if bfs(x1, y1) {
				out.YesNo(true)
