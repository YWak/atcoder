...
func calc() {
	...
	// i番目を飛ばしたときの操作は、つぎのことがわかれば解決する
	// i-1番目までの操作で1がどこにいるか (B[k]とする)
	// i+1番目以降でB[k]にある要素は、最終的にどこにいるか

	// i番目の要素は、最終的にlast[i]にたどり着く
	last := make([]int, n)
	for i := 0; i < n; i++ {
		b[i] = i + 1
		last[i] = i + 1
	...
	for i := range a {
		v := a[len(a)-1-i]
		last[v-1], last[v] = last[v], last[v-1]
	}

	i1 := 0
	for _, v := range a {
		// i番目の操作を飛ばした場合、今1がいる場所の要素が最終的にどこに行くかを調べる。
		// lastの操作をキャンセルする。2回やるともとに戻る
		last[v-1], last[v] = last[v], last[v-1]
