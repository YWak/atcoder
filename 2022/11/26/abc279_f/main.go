...
func calc() {
	n, Q := in.NextInt2()
	box := make([]map[int]bool, n+1)

	// 箱のインデックス。
	// box[x]を参照しろと言われたら、実際はbox[bi[x]]を参照することにする
	// rbiは逆で、rbi[bi[x]] = xとなるようにする
	bi := make([]int, n+1)
	rbi := make([]int, n+1)
	for i := range box {
		box[i] = map[int]bool{i: true}
		bi[i] = i
		rbi[i] = i
	}
	...
	for q := 0; q < Q; q++ {
		...
		if op == 1 {
			x, y := in.NextInt2()
			if len(box[bi[x]]) < len(box[bi[y]]) {
				// fromに複製して、インデックスをすり替える。
				bi[x], bi[y] = bi[y], bi[x]
				rbi[bi[x]] = x
				rbi[bi[y]] = y
			}
