...
func solve() int {
	n2, n3, n4 := in.NextInt3()

	pat := []func(n2, n3, n4 int) (int, int, int, int){
		func(n2, n3, n4 int) (int, int, int, int) {
			// n3 *2 + n4 *1
			a := min(n3/2, n4)
			return a, 0, a * 2, a
		},
	...
	ans := 0
	ps := make(Permutation, len(pat))
	for i := range ps {
		ps[i] = i
	}
	for {
		_n2, _n3, _n4 := n2, n3, n4
		a := 0
		for _, i := range ps {
			t1, t2, t3, t4 := pat[i](_n2, _n3, _n4)
			a += t1
			_n2 -= t2
			_n3 -= t3
			_n4 -= t4
