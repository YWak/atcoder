...
const mod = mint(1000*1000*1000 + 7)
...
func calc() {
	...
	// (P(d,l, x*y-d-l) - P(d, l, (x-1)*(y-1) - d-l)) * (r - y + 1) * (c - x + 1)
	// pはx*yの中にd個のデスクとl個のサーバーを配置する場合の数
	p := func(x, y int) int {
		if x <= 0 || y <= 0 || d+l > x*y {
			return 0
		}
		ret := fact[x*y]
		mod.chdiv(&ret, fact[d])
		mod.chdiv(&ret, fact[l])
		mod.chdiv(&ret, fact[x*y-d-l])
		return ret
	}
	...
	for b := 0; b < 1<<4; b++ {
		...
		if popcount(b)%2 == 0 {
			mod.chadd(&pat1, p(_x, _y))
		} else {
			mod.chsub(&pat1, p(_x, _y))
		}
	}

	pat2 := (r - x + 1) * (c - y + 1)
	out.Println(mod.mul(pat1, pat2))
}
