...
func calc() {
	...
	next := NewIntInt(2, n*2, INF18) // 次の0/1が何番目にあるか？
	for i := len(s) - 1; i >= 0; i-- {
		v := s[i]
		for u := range next {
			if i+1 < len(next[u]) {
	...
	prev := NewIntInt(2, n*2, INF18) // 直前の0/1が何番目にあるか？
	for i, v := range s {
		for u := range prev {
			if i-1 >= 0 {
				prev[u][i] = prev[u][i-1]
