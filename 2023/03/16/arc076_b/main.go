...
func calc() {
	...
	// 指定したフィールドでソートし、前後を変でつなぐ
	do := func(get func(p *tri) int) {
		sort.Slice(ps, func(i, j int) bool { return get(ps[i]) < get(ps[j]) })
		for i := range ps {
			if i > 0 {
				es = append(es, &edge{ps[i].i, ps[i-1].i, abs(get(ps[i]) - get(ps[i-1]))})
			}
			if i < n-1 {
				es = append(es, &edge{ps[i].i, ps[i+1].i, abs(get(ps[i+1]) - get(ps[i]))})
			}
		}
	}
	do(func(p *tri) int { return p.x })
	do(func(p *tri) int { return p.y })
