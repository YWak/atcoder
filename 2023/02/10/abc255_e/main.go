...
func calc() {
	...
	// a[0] == 0としたときのaを作る。c[t]はtの数。ksはtを昇順にソートしたもの
	a := make([]int, n)
	plusA0 := map[int]int{}
	minusA0 := map[int]int{}
	...
	// candの候補は、cplusとcminusがx[i]になるようにするパターンだけ考えればいい
	cand := map[int]int{}
	for _, x := range xs {
		for c := range plusA0 {
			// c+a[0] = x になるようなa[0]
	...
	for a0 := range cand {
		_ans := 0
		// a[0] == cとしたときの各値がxに当てはまるかを調べる
		for _, x := range xs {
			_ans += plusA0[x-a0]  // x = a[i] = c + a[0]になる
			_ans += minusA0[x+a0] // x = a[i] = c - a[0]になる
		}
		chmax(&ans, _ans)
	}
