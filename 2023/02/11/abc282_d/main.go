...
var done = [200000]bool{}

func dfs(g [][]int, curr, prev, color int, nodecolor *map[int]int, color2node *[2][]int) {
	(*nodecolor)[curr] = color
	(*color2node)[color] = append((*color2node)[color], curr)
	done[curr] = true
	...
	for _, next := range g[curr] {
		...
		dfs(g, next, curr, 1-color, nodecolor, color2node)
	}
}
...
func calc() {
	...
	for i, l := range groups {
		nodecolor := map[int]int{}
		color2node := [2][]int{}
		dfs(g, i, -1, 0, &nodecolor, &color2node)
		sort.Ints(color2node[0])
		sort.Ints(color2node[1])

		for _, u := range l {
			cn := color2node[1-nodecolor[u]]
			ok, ng := -1, len(cn)
			for abs(ok-ng) > 1 {
				mid := (ok + ng) / 2
				if cn[mid] < u {
			...
			c := len(cn) - (ok + 1)
			for _, v := range g[u] {
				if u < v && nodecolor[u] != nodecolor[v] {
					c--
				}
			}
			ans += c
