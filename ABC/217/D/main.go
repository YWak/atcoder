...
func calc() {
	l, q := in.NextInt2()
	queries := make([]pair, 0, q)
	blocks := make(sort.IntSlice, 0, q)
	blocks = append(blocks, 0, l)

	for i := 0; i < q; i++ {
		c, x := in.NextInt2()
		queries = append(queries, pair{c, x})
		if c == 1 {
	...
	for i := q - 1; i >= 0; i-- {
		...
		if query.c == 1 {
			// つなぐ
			uf.Unite(indexOf[query.x], indexOf[query.x]+1)
		} else {
			ok, ng := -1, len(blocks)
			for abs(ok-ng) > 1 {
				mid := (ok + ng) / 2
				if blocks[mid] <= query.x {
					ok = mid
				} else {
					ng = mid
				}
			}
			ans = append(ans, uf.Size(ok))
		}
	}
