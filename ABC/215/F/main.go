...
func (a Points) Less(i, j int) bool { return a[i].x < a[j].x || a[i].x == a[j].x && a[i].y < a[j].y }
...
func calc() {
	...
	for i := 0; i < n; i++ {
		x, y := in.NextInt2()
		ps[0] = append(ps[0], Point{x, y})
		ps[1] = append(ps[1], Point{x, y})
	}
	...
	for k := 0; k < 2; k++ {
		ps[k] = append(ps[k], Point{INF18, INF18})
		sort.Sort(ps[k])
		maxls[k][0] = -INF18
		for i := 0; i < n; i++ {
			maxls[k][i+1] = max(ps[k][i].y, maxls[k][i])
		}
		minrs[k][n] = INF18
		for i := n - 1; i >= 0; i-- {
			minrs[k][i] = min(ps[k][i].y, minrs[k][i+1])
		}
	}

	// 距離がm以上になる点があるか
	check := func(k, m int) bool {
		p := ps[k]
		minr = minrs[k]
		maxl = maxls[k]
		...
		for i := 0; i < n; i++ {
			// abs(p[r].x - p[i].x) >= mになるまでrを進める
			for abs(p[r].x-p[i].x) < m {
				r++
			}
			// abs(p[l].x - p[i].x) <= mになるlを求める
			for abs(p[l+1].x-p[i].y) >= m {
				l++
			}
