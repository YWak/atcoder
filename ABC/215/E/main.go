...
func calc() {
	...
	s := make([]int, n)
	for i := 0; i < n; i++ {
		s[i] = int(ss[i] - 'A') // 0はまだどこも出ていないとき
	}

	x := C //未踏

	// dp[S][c][i]はi番目まででSパターンを実現していて最後にcに出場したときの場合の数
	dp := make([][][]int, 1<<C)
	for i := 0; i < (1 << C); i++ {
		dp[i] = make([][]int, C+1)
		for j := 0; j < C+1; j++ {
	...
	for i := 0; i < n; i++ {
		for S := 0; S < (1 << C); S++ {
			for c := 0; c <= C; c++ {
				...
				dp[S][c][i+1] = (dp[S][c][i+1] + now) % mod // 参加しないケースを足す

				if (S>>si)%2 == 0 || c == si || c == x { // まだ参加したことがない、参加中、新規参加なら参加できる
					dp[S_][si][i+1] = (dp[S_][si][i+1] + now) % mod // i番目に参加する
				}
			}
		}
	}
