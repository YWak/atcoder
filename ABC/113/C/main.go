...
func main() {
	...
	P := make([]pref, M)
	for i := 0; i < M; i++ {
		P[i] = pref{i, nextInt(), nextInt(), ""}
	}
	sort.Slice(P, func(i, j int) bool { return P[i].p < P[j].p || P[i].p == P[j].p && P[i].y < P[j].y })
	prev := -1
	no := 0
	for i := 0; i < M; i++ {
		if P[i].p != prev {
		...
		P[i].code = fmt.Sprintf("%06d%06d", P[i].p, no)
		no++
		prev = P[i].p
	}
	sort.Slice(P, func(i, j int) bool { return P[i].i < P[j].i })
	for i := 0; i < M; i++ {
		fmt.Println(P[i].code)
	}
}
