...
func calc() {
	...
	s := make([]Pointf, n)
	t := make([]Pointf, n)
	gsx, gsy, gtx, gty := 0.0, 0.0, 0.0, 0.0
	...
	// 	return math.Atan2(s[i].y, s[i].x) < math.Atan2(s[j].y, s[j].x)
	// })
	// sort.Slice(t, func(i, j int) bool {
	// 	return math.Atan2(t[i].y, t[i].x) < math.Atan2(t[j].y, t[j].x)
	// })
	p := make([]*Pointf, n)
	for i := 0; i < len(s); i++ {
		for j := 0; j < len(t); j++ {
			...
			if d1 < eps || math.Abs(d1-d2) > eps {
				continue
			}
			th := math.Atan2(t[j].y, t[j].x) - math.Atan2(s[i].y, s[i].x)
			ok := true
			for k := 0; k < n; k++ {
				p[k] = s[k].rotate(th)
			}
			for k := 0; k < n; k++ {
				found := false
				for l := 0; l < n; l++ {
					if math.Abs(t[l].x-p[k].x) < eps && math.Abs(t[l].y-p[k].y) < eps {
						found = true
						break
					}
				}
...
func (p *Pointf) dist(o Pointf) float64 {
	return math.Sqrt(math.Pow(p.x-o.x, 2) + math.Pow(p.y-o.y, 2))
}

func (p *Pointf) dist0() float64 {
	return math.Sqrt(math.Pow(p.x, 2) + math.Pow(p.y, 2))
}

func (p *Pointf) rotate(th float64) *Pointf {
	return &Pointf{p.x*math.Cos(th) - p.y*math.Sin(th), p.x*math.Sin(th) + p.y*math.Cos(th)}
}
