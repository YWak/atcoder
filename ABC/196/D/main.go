...
func dfs(state, p, a, b int) int {
	...
	if state&curr > 0 {
		// おいてある場合はそのまま
		return dfs(state, p+1, a, b)
	}

	ans := 0
	if b > 0 {
		// 半畳をおく
		ans += dfs(state|curr, p+1, a, b-1)
	}
	if a > 0 && h < H-1 {
		// 縦置き
		ans += dfs(state|curr|down, p+1, a-1, b)
	}
	// 横置き
	if a > 0 && w < W-1 && (state&right) == 0 {
		ans += dfs(state|curr|right, p+1, a-1, b)
	}

	return ans
}

func debug1(state int) {
	row := make([]int, W)
	for i := 0; i < H; i++ {
		for j := 0; j < W; j++ {
			if state&(1<<((i+1)*W+j)) > 0 {
				row[j] = 1
			}
		}
		debug(i+1, row)
