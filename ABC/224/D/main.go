...
func calc() {
	...
	start := []byte("000000000")
	for i := 0; i < 8; i++ {
		p := in.NextInt()
		start[p-1] = '0' + byte(i+1)
	}
	h[string(start)] = 0
	queue := pqNew()
	queue.Push(state{0, string(start)})
	for !queue.Empty() {
		q := queue.Pop()
		...
		for _, v := range g[u] {
			...
			s, e := h[next]
			if !e || s > q.step+1 {
				queue.Push(state{q.step + 1, next})
				h[next] = q.step + 1
			}
		}
	}
...
func (pq PQList) prior(i, j int) bool {
	return pq[i].step < pq[j].step // 大きいもの優先とする
}
