...
func main() {
	...
	count := func(a, b int) mint {
		a2, b2 := mint(2).pow(a), mint(2).pow(b)
		return a2.add(b2).sub(mint(1))
	}
	// debug()
	// debug(m)
	a := mint(1)
	...
	a = a.sub(mint(1)) // どれも選ばない場合を引く

	fmt.Println(a.add(mint(f00)))
}

func rev(f pair) pair {
	return pair{f.s * -1, f.b, f.a}
}
func frac(a, b int) pair {
	d := gcd(abs(a), abs(b))
	return pair{a / abs(a) * b / abs(b), abs(a) / d, abs(b) / d}
}
