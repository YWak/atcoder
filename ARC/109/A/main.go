...
func main() {
	...
	graph := NewGraph(200)
	A := func(n int) int { return n }
	B := func(n int) int { return n + 100 }

	for i := 0; i < 100; i++ {
		graph.AddWeightedEdge(A(i), B(i), x)
		graph.AddWeightedEdge(B(i), A(i), x)
	}
	for i := 0; i < 99; i++ {
		graph.AddWeightedEdge(A(i+1), B(i), x)
		graph.AddWeightedEdge(B(i), A(i+1), x)
		graph.AddWeightedEdge(A(i), A(i+1), y)
		graph.AddWeightedEdge(A(i+1), A(i), y)
		graph.AddWeightedEdge(B(i), B(i+1), y)
