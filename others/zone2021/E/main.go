...
func main() {
	...
	for r := 0; r < R; r++ {
		for c := 0; c < C; c++ {
			// ノードをつなぐ
			if c+1 < C {
				graph.AddWeightedEdge(ref(r, c), ref(r, c+1), A[r][c])
			}
			if c-1 >= 0 {
				graph.AddWeightedEdge(ref(r, c), ref(r, c-1), A[r][c-1])
			}
			if r+1 < R {
	...
	ans, route := graph.Dijkstra(ref(0, 0), ref(R-1, C-1))
	if false {
		rev := func(k int) (int, int) { return k / C, k % C }
		for i := 0; i < len(route)-1; i++ {
			a, b := rev(route[i])
			x, y := rev(route[i+1])
			cost := -1
			for _, v := range graph.list[route[i]] {
				if v.to == route[i+1] {
					cost = v.weight
				}
			}

			debug(a+1, b+1, "=>", x+1, y+1, "=", cost)
		}
	}
	fmt.Println(ans)
}
