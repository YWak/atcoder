	...
	for i := 0; i < q; i++ {
		...
		if t == 1 {
			...
			for _, d := range [][]int{{-1, +0}, {+1, +0}, {+0, -1}, {+0, +1}} {
				...
				if x < 0 || y < 0 || x >= h || y >= w || color[x][y] == 0 {
					continue
				}

				uf.Unite(index(r, c), index(x, y))
			}
		} else {
			ra, ca := in.NextInt2d(-1, -1)
			rb, cb := in.NextInt2d(-1, -1)

			if ra == rb && ca == cb {
				out.YesNo(color[ra][ca] == 1)
			} else {
				out.YesNo(uf.Same(index(ra, ca), index(rb, cb)))
			}
		}
	}
}
