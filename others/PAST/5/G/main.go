...
func main() {
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if field[i][j] == '#' {
				queue = append(queue, item{pair{i, j}, 1 << (i*w + j), []pair{{i, j}}})
			}
		}
	}
	...
	for len(queue) > 0 {
		...
		if len(q.hist) == k {
			fmt.Println(k)
			for _, p := range q.hist {
				fmt.Println(p.h+1, p.w+1)
			}
			return
		}
		for _, d := range dir {
			x, y := q.c.h+d.h, q.c.w+d.w
			if x < 0 || y < 0 || x >= h || y >= w {
				continue
			}
			if field[x][y] == '.' {
			...
			hist2 := make([]pair, len(q.hist)+1)
			for i := 0; i < len(q.hist); i++ {
				hist2[i].h, hist2[i].w = q.hist[i].h, q.hist[i].w
			}
			hist2[len(q.hist)].h, hist2[len(q.hist)].w = x, y
			next := item{pair{x, y}, q.used | s, hist2}
