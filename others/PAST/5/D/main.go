...
func main() {
	...
	sort.Slice(ss, func(i, j int) bool {
		li, lj := leading(ss[i]), leading(ss[j])
		if len(ss[i])-li != len(ss[j])-lj {
			return len(ss[i])-li < len(ss[j])-lj
		}
		for ii, jj := li, lj; ii < len(ss[i]); ii, jj = ii+1, jj+1 {
			if ss[i][ii] != ss[j][jj] {
				return ss[i][ii] < ss[j][jj]
			}
		}
		return li > lj
	})
