...
func calc() {
	...
	used := NewIntInt(n+1, x+1, 0) // 使用済みか？
	dp1 := NewIntInt(n+1, x+1, 0)  // i番目の袋まで見て、銅貨がj枚あるときの最大の金貨の枚数
	dp2 := NewIntInt(n+1, x+1, 0)  // dp1の状況のときの銀貨の枚数
	dp2[0][x] = int(1e9)
	used[0][x] = 1
	...
	for i, b := range boxes {
		for j := 0; j <= x; j++ {
			...
			ii := i + 1
			// 買わない場合
			if dp1[ii][j] < dp1[i][j] || dp1[ii][j] == dp1[i][j] && dp2[ii][j] < dp2[i][j] {
				used[ii][j] = 1
				dp1[ii][j], dp2[ii][j] = dp1[i][j], dp2[i][j]
			}
			// 買う場合
			jj := j - b.b
			gold, silver := dp1[i][j]+b.c, dp2[i][j]-b.a
			if jj >= 0 && silver >= 0 && (dp1[ii][jj] < gold || dp1[ii][jj] == gold && dp2[ii][jj] < silver) {
				used[ii][jj] = 1
				dp1[ii][jj], dp2[ii][jj] = gold, silver
			}
		}
	}
	...
	p, q, r := 0, 0, 0
	for i := 0; i <= x; i++ {
		p1, q1, r1 := dp1[n][i], dp2[n][i], i
		if p > p1 || p == p1 && q > q1 || p == p1 && q == q1 && r > r1 {
			continue
		}
		p, q, r = p1, q1, r1
	}
