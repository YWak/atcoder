...
func calc() {
	...
	sort.Slice(ss, func(i, j int) bool { return ss[i] < ss[j] }) // 一個使って最小
	m := make([]map[uint64]string, 8)
	for i := 0; i < len(m); i++ {
		m[i] = map[uint64]string{}
	}
	...
	for i := 1; i < len(m); i++ {
		for u, su := range m[i-1] {
			for v, sv := range m[i-1] {
				if (u & v) == 0 {
					var s string
					if su < sv {
						s = su + sv
	...
	for i := 7; i >= 0; i-- {
		...
		mins := "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz"
		minu := uint64(0)
		for u, s := range m[i] {
			if s < mins && (u&used) == 0 {
				mins = s
