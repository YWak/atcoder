...
func calc() {
	...
	for i := 0; i < len(x); i++ {
		st.Add(i+1, len(x), x[i])
	}
	// debug(st)
	ans := make([]int, 0, len(x)+10)
	r := 0
	for i := len(x) - 1; i >= 0; i-- {
		v := st.Get(i)
		ans = append(ans, (r+v)%10)
		r = (r + v) / 10
	}
	for r > 0 {
		ans = append(ans, r%10)
...
// Add は セグメントツリーの from から to までに値valueを加算します。
func (st SegmentTree) Add(from, to, value int) {
	st.update1(from, to, 0, 0, len(st)/2, value)
}

func (st SegmentTree) update1(from, to, k, l, r, value int) {
	// debug(from, to, k, l, r)
	if r <= from || to <= l {
		// 範囲外なら無視
		return
	...
	m := (l + r) / 2
	st.update1(from, to, k*2+1, l, m, value)
	st.update1(from, to, k*2+2, m, r, value)
}
