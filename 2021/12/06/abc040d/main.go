...
func calc() {
	...
	edges := make([]edge, 0, n)
	for i := 0; i < m; i++ {
		a, b, y := in.NextInt3d(-1, -1, 0)
		edges = append(edges, edge{a, b, y})
	}
	t := in.NextInt()
	query := make([]pair, 0, t)
	for i := 0; i < t; i++ {
		v, w := in.NextInt2d(-1, 0)
		query = append(query, pair{i, v, w})
	}
	...
	// 大きい順
	sort.Slice(edges, func(i, j int) bool { return edges[i].y > edges[j].y })
	sort.Slice(query, func(i, j int) bool { return query[i].w > query[j].w })
