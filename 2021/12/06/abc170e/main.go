...
func calc() {
	...
	n, q := in.NextInt2()
	kinders := make([]*Treap, 200000)
	for i := 0; i < len(kinders); i++ {
		kinders[i] = NewRevIntTreap()
		kinders[i].allowDup = true
	}

	rates := make([]int, n)
	now := make([]int, n)
	for i := 0; i < n; i++ {
		a, b := in.NextInt2d(0, -1)
		rates[i] = a
		now[i] = b
	...
	seg := make(SegmentTree, len(kinders))
	seg.Init()
	...
	for i := 0; i < len(kinders); i++ {
		...
		if kinders[i].Len() == 0 {
			...
			key, _ := kinders[i].GetKth(k)
			v = key.(int)
		}
		seg.Update(i, v)
	}
	for i := 0; i < q; i++ {
		c, to := in.NextInt2d(-1, -1)
		from := now[c]
		now[c] = to

		// debug(c+1, from+1, to+1)
		// 移動
		kinders[from].Remove(rates[c])

		var fromRate int
		if kinders[from].Len() == 0 {
			fromRate = INF18
		} else {
			fr, _ := kinders[from].GetKth(k)
			fromRate = fr.(int)
		}
		seg.Update(from, fromRate)

		kinders[to].Put(rates[c], 1)
		toRate, _ := kinders[to].GetKth(k)
		seg.Update(to, toRate.(int))

		// debug(from, fromRate, to, toRate)
		out.Println(seg.Query(0, 200000))
		// for j := 0; j < 5; j++ {
		// 	debug(j, kinders[j].Iterate())
		// }
	}
}
...
func (t *Treap) Iterate() []interface{} {
	return t._iterate(t.root)
}

func (t *Treap) _iterate(n *node) []interface{} {
	if n == nil {
		return []interface{}{}
	}
	...
	ret := make([]interface{}, 0, count+1)
	if n.left != nil {
		ret = append(ret, t._iterate(n.left)...)
	}
	ret = append(ret, n.key)
	if n.right != nil {
		ret = append(ret, t._iterate(n.right)...)
	}
