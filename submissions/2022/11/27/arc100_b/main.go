...
func calc() {
	...
	// [l, r)の範囲でa[l:t] < a[t:r]となるtを探して返す。
	seq := func(l, r int) int {
		ll, rr := l, r
		for abs(rr-ll) > 1 {
			m := (ll + rr) / 2
			sl := sum[m] - sum[l]
	...
	ans := INF18
	for i := 2; i < n-1; i++ {
		// a[0:i]とa[i:n]に分割して、それぞれをP,QとR,Sに分割する。
		// P<Qになるギリギリの点jと、R<Sになるギリギリの点kを求める
		j := seq(0, i)
		k := seq(i, n)

		// {j+0,j+1} x {k+0, k+1}の組み合わせで最小になるか確認する
		for dj := 0; dj < 2; dj++ {
			for dk := 0; dk < 2; dk++ {
				jj := j + dj
				kk := k + dk
