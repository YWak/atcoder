...
func dfs(used, prev, player int) int {
	other := 1 - player
	p := &memo[used][prev][player]
	if *p == -1 {
		// 使用できる手について、どれかでplayerが勝つなら勝利、さもなくば負け
		*p = other
		for i := 0; i < n; i++ {
			// 一度使ったらもう使えない、接続されていなければ使えない
			if nthbit(used, i) == 1 || g[prev][i] == 0 {
				continue
			}
			if dfs(used|1<<i, i, other) == player {
				*p = player
				break
			}
		}
...
func calc() {
	...
	// dp[used][i] は、使用済みの文字列がusedで、最後の文字列がiのときの
	// ただし、初手はi==n
	memo = make([][][]int, 1<<n)
	for i := range memo {
		memo[i] = make([][]int, n+1)
	...
	out.Println(chs(dfs(0, n, 0) == 0, "First", "Second"))
}
