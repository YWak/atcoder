...
func add(m *Treap, rate interface{}) {
	cnt := m.Get(rate)
	if cnt == nil {
		m.Put(rate, 1)
	} else {
		m.Put(rate, cnt.(int)+1)
	}
}

func remove(m *Treap, rate interface{}) {
	cnt := m.Get(rate).(int)
	if cnt == 1 {
		m.Remove(rate)
	} else if cnt == 0 {
		panic("rate is not found")
	} else {
		m.Put(rate, cnt-1)
	}
}
...
func calc() {
	...
	kinders := make([]*Treap, 200001)
	equality := NewIntTreap()
	...
	for i := 0; i < q; i++ {
		c, d := in.NextInt2()
		from := kinders[belongs[c]]
		to := kinders[d]

		if from.Len() > 0 {
			rate, _ := from.GetKth(1)
			remove(equality, rate)
		}
		if to.Len() > 0 {
			rate, _ := to.GetKth(1)
			remove(equality, rate)
...
func (t *Treap) Keys() []interface{} {
	return t._keys(t.root)
}

func (t *Treap) _keys(n *node) []interface{} {
	if n == nil {
		return []interface{}{}
	}

	keys := make([]interface{}, 0)
	keys = append(keys, t._keys(n.left)...)
	keys = append(keys, n.key)
	keys = append(keys, t._keys(n.right)...)
