...
func score(s, l, r int) int {
	if s <= l { // 完全に右側
		return (r - l + 1) * (l - s + r - s) / 2
	} else if r <= s { // 完全に左側
		return (r - l + 1) * (s - r + s - l) / 2
	}
	// 両サイド
	return (s-l)*(s-l+1)/2 + (r-s)*(r-s+1)/2
}
...
func calc() {
	...
	for gl := -300; gl <= 300; gl++ {
		...
		br = bl + b - 1
		s := score(-100, rl, rr) + score(0, gl, gr) + score(+100, bl, br)
		if s < ans {
			ans = s
			// debug(s, rl, rr, gl, gr, bl, br)
		}
	}
	out.Println(ans)
}
