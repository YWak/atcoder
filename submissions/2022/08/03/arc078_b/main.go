...
func dfs1(g [][]int, curr, prev int) []int {
	if curr == 0 {
		return []int{curr}
	}
...
func dfs2(g [][]int, curr, prev, p1, p2 int) int {
	ans := 1
	for _, next := range g[curr] {
		if next == prev {
			continue
...
func calc() {
	...
	path := dfs1(g, n-1, -1)
	m := (len(path) - 1) / 2
	p1, p2 := path[m], path[m+1]

	ans := dfs2(g, 0, -1, p1, p2) - dfs2(g, n-1, -1, p1, p2)
	out.Println(chs(ans > 0, "Fennec", "Snuke"))
}
