...
func calc() {
	...
	sort.Slice(keys, func(i, j int) bool { return unused[keys[i]] > unused[keys[j]] })

	score := func() int { return s + pow(len(types), 2) }
	ans := score()
	for _, t := range keys {
		if types[t] {
			continue
