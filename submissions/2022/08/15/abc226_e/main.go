...
func _dfs(g [][]int, curr, prev int, nhist, ehist map[int]bool) {
	nhist[curr] = true
	...
	for _, next := range g[curr] {
		...
		e := min(curr, next)*N10_6 + max(curr, next)
		if ehist[e] {
			// 来たことがあれば無視
			continue
		}
...
func calc() {
	...
	for i := 0; i < n; i++ {
		...
		nhist, ehist := dfs(g, i, -1)
		for u := range nhist {
			used[u] = true
		}

		if len(nhist) == len(ehist) {
			ans *= 2
		} else {
			ans *= 0
		}
