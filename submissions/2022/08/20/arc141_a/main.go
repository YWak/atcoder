...
func solve() {
	...
	parseInt := func(v string) int {
		u, _ := strconv.ParseInt(v, 10, 64)
		return int(u)
	}

	for l := n; l > 0; l-- {
		if l != n {
			s = strings.Repeat("9", l)
		}
		for i := 1; i < l; i++ {
			if l%i != 0 {
				continue
			...
			unit1 := s[0:i]
			base := parseInt(unit1)
			unit2 := fmt.Sprint(base - 1)

			ans1 := strings.Repeat(unit1, k)
			ans2 := strings.Repeat(unit2, k)

			if len(ans1) == l && ans1 <= s {
				chmax(&ans, parseInt(ans1))
			}
			if len(ans2) == l && ans2 <= s {
				chmax(&ans, parseInt(ans2))
			}
		}
	}
