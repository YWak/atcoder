...
func calc() {
	...
	for i := range dist {
		dist[i] = make([]float64, len(ps))
		for j := range dist {
			x, y := ps[i].x-ps[j].x, ps[i].y-ps[j].y
	...
	speed := func(s int) float64 {
		return sp[popcount(s&boosts)]
	}
	...
	for s := 0; s < 1<<nm; s++ {
		for i := 0; i < nm; i++ {
			...
			for j := 0; j < nm; j++ {
				...
				next := &dp[s|1<<j][j]
				*next = math.Min(*next, dp[s][i]+dist[i][j]/speed(s))
			}
		}
	}
	ans := math.MaxFloat64
