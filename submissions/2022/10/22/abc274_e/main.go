...
func calc() {
	...
	for i := range dists {
		dists[i] = make([]float64, len(ps))
		for j := range dists[i] {
			x := ps[i].x - ps[j].x
			y := ps[i].y - ps[j].y
			dists[i][j] = math.Sqrt(x*x + y*y)
		}
	}

	// dp[s][i][b] は sに到着、i番目にいてブーストの数がbのときの最小時刻
	fmax := math.MaxFloat64 / 100
	dp := make([][][]float64, 1<<N)
	for i := range dp {
		dp[i] = make([][]float64, O)
		for j := range dp[i] {
			dp[i][j] = make([]float64, m+1)
			for k := range dp[i][j] {
				dp[i][j][k] = fmax
			}
		}
	}
	// 原点からある点に向かったときの距離が初期値
	for i := 0; i < N; i++ {
		if i < n {
			dp[1<<i][i][0] = dists[i][O]
		} else {
			dp[1<<i][i][1] = dists[i][O]
		}
	}
	...
	for s := 0; s < 1<<N; s++ {
		for i := 0; i < N; i++ {
			...
			for j := 0; j < N; j++ {
				...
				for b := 0; b <= m; b++ {
					if popcount(s&boosts) != b {
						continue
					}
					next := dp[s][i][b] + dists[i][j]*speed[b]
					bb := b
					if j >= n {
						bb = b + 1
					}
	...
	for s := 0; s < 1<<(m+n); s++ {
		...
		for i := 0; i < n+m; i++ {
			for b := 0; b <= m; b++ {
				ans = math.Min(ans, dp[s][i][b]+dists[i][O]*speed[b])
			}
		}
	}
	out.Printf("%.10f\n", ans)
