...
func calc() {
	mod := Mod1000000007()
	s := in.NextBytes()
	chokudai := []byte("*chokudai")
	dp := make([][]int, len(s)+1)
	for i := range dp {
		dp[i] = make([]int, len(chokudai)+1)
	}
	dp[0][0] = 1
	for i, c := range s {
		for j := range chokudai {
			// 使わないパターン
			mod.chadd(&dp[i+1][j], dp[i][j])

			// 使うパターン
			if j < len(chokudai)-1 && c == chokudai[j+1] {
				mod.chadd(&dp[i+1][j+1], dp[i][j])
			}
		}
	}

	out.Println(dp[len(s)][len(chokudai)-1])
}
