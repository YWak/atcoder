...
func dfs(group []int, i int, colors map[int]int) int {
	if i == len(group) {
		return 1
	}
...
func list(u int, hist map[int]bool) []int {
	ans := []int{u}
	hist[u] = true
	for _, v := range graph[u] {
		if hist[v] {
