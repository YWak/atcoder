...
func solve() {
	...
	for i := 0; i <= (n+1)/2; i++ {
		dp[i+1][1] = mod.add(dp[i+1][1], mod.mul(dp[i][1], 26)) // 小さいことが確定していればどれを使用しても良い
		if s[i] > 0 {                                           // 小さいことが確定していなくても、それより小さい値を使用すれば確定する
			dp[i+1][1] = mod.add(dp[i+1][1], mod.mul(dp[i][0], 26-s[i]-1))
		}
		dp[i+1][0] = mod.add(dp[i+1][0], dp[i][0])
	}

    ans := mod.add(dp[][0], dp[][1])
