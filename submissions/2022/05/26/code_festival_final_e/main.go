...
func calc() {
	...
	// dp[i][j][k]は i番目まで見たときに、最後の値がr[j]で、直前がk(0:減少, 1:増加)のときの最大数
	dp := [3001][3001][2]int{}
	for i := 0; i < n; i++ {
		// 初めて使うパターン
		dp[i+1][i][0] = max(dp[i+1][i][0], 1)
		dp[i+1][i][1] = max(dp[i+1][i][1], 1)
		...
		for j := 0; j < i; j++ {
			...
			if r[i] > r[j] {
				dp[i+1][i][1] = max(dp[i+1][i][1], dp[i][j][0]+1)
			} else if r[i] < r[j] {
				dp[i+1][i][0] = max(dp[i+1][i][0], dp[i][j][1]+1)
			}
		}
	}
	ans := 0
