...
func calc() {
	...
	pq := NewIntPriorityQueue(func(a, b *node) bool {
	...
	for !pq.Empty() {
		no := pq.Pop()
		if costs[no.to] < no.cost {
			continue
		}
		costs[no.to] = no.cost
		for _, next := range g[no.to] {
			c := no.cost + 1
			if costs[next] > c {
				costs[next] = c
				pq.Push(&node{next, c})
...
func NewIntPriorityQueue(prior func(a, b *node) bool) PriorityQueue {
...
func (list *PriorityQueueList) Push(item interface{}) {
	list.values = append(list.values, item.(*node))
}
