...
func connected(p int) bool {
	block := 0
	hole := 0
	isOut := func(i, j int) bool { return i < 0 || i == 4 || j < 0 || j == 4 }
...
func show(p, a int) {
	for i := 0; i < 4; i++ {
		...
		buf2 := []int{}
		for j := 0; j < 4; j++ {
			buf1 = append(buf1, ch(p&(1<<(i*4+j)) != 0, 1, 0))
		}
		for j := 0; j < 4; j++ {
			buf2 = append(buf2, ch(a&(1<<(i*4+j)) != 0, 1, 0))
		}
		debug(buf1, buf2)
	}
	debug()
