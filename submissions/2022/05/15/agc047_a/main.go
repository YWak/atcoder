...
func calc() {
	...
	for i := 0; i < n; i++ {
		...
		if dot == -1 {
			bottom = 1
			top, _ = strconv.Atoi(s)
		} else {
			n, _ := strconv.Atoi(s[0:dot])
			f, _ := strconv.Atoi(s[dot+1:])
			bottom = pow(10, len(s)-dot-1)
			top = n*bottom + f
		}
		s2t, s5t := divcount(top, 2), divcount(top, 5)
		s2b, s5b := divcount(bottom, 2), divcount(bottom, 5)
		c[s2t-s2b+geta][s5t-s5b+geta]++
	}
