...
func calc() {
	...
	for i, o := range ps {
		...
		for j, p := range ps {
			if i != j {
				pp = append(pp, &Point{p.x - o.x, p.y - o.y})
			}
		}
		m := len(pp)
		geo.HenkakuSort(pp)
		for l := 0; l < m; l++ {
			...
			ans = math.Max(ans, geo.Angle2(pp[l], pp[ok]))
			ans = math.Max(ans, geo.Angle2(pp[l], pp[(ok+1)%m]))
		}
	}
...
func (p *Point) Radian() float64 {
	a := math.Atan2(float64(p.y), float64(p.x))
	if a < 0 {
		a += math.Pi * 2
	}
	return a
...
func (p *Point) Degree() float64 {
	return p.Radian() / math.Pi * 180
}
...
// Angle2は2点の偏角の差を0<=a<=2*Piで返します。
func (g *Geo) Angle2(p1 *Point, p2 *Point) float64 {
	t1 := p1.Radian()
	t2 := p2.Radian()
	return math.Min(math.Abs(t1-t2), math.Pi*2-math.Abs(t1-t2))
}
