...
func calc() {
	...
	// dp[i][j] はi人目の子供まで配って残りがj個の場合の数
	dp := NewIntInt(n+1, k+1, 0)
	dp[0][0] = 1
	for i := 0; i < n; i++ {
		// -- O(nk^2) で死ぬパターン
		// for v := 0; v <= a[i]; v++ {
		// for j := 0; j+v <= k; j++ {
		// 	mod.chadd(&dp[i+1][j+v], dp[i][j])
		// }
		// }
		// -- addの1項目を固定にするがまだO(nk^2)
		// for v := 0; v <= a[i]; v++ {
		// for j := v; j <= k; j++ {
		// 	mod.chadd(&dp[i+1][j], dp[i][j-v])
		// }
		// }
		// 累積和を使った高速化 [0, k-v]の和を高速に設定していく
		s := make([]int, k+2)
		for j := 0; j <= k; j++ {
			s[j+1] = mod.add(s[j], dp[i][j])
		}
		for j := 0; j <= k; j++ {
			// [max(0, j-v), j]を採用する
			mod.chadd(&dp[i+1][j], mod.sub(s[j+1], s[max(0, j-a[i])]))
		}
	}
