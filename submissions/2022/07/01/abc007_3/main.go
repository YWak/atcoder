...
func calc() {
	r, c := in.NextInt2()
	sr, sc := in.NextInt2d(-1, -1)
	gr, gc := in.NextInt2d(-1, -1)
	...
	for len(queue) > 0 {
		...
		for t := 0; t < 4; t++ {
			i, j := p.r+dr[t], p.c+dc[t]
			if i == -1 || i == r || j == -1 || j == c || f[i][j] == '#' {
				continue
			}
			if cost[i][j] <= cost[p.r][p.c]+1 {
				continue
			}
			cost[i][j] = cost[p.r][p.c] + 1
			queue = append(queue, &pair{i, j})
		}
	}
	out.Println(cost[gr][gc])
}
