...
func calc() {
	...
	// cost[i][j]は [i, j)までのスライムを合体させたときの最小コスト
	cost := NewIntInt(n+1, n+1, INF18)
	weight := make([]int, n+1)
	for i := 0; i < n; i++ {
		weight[i+1] = weight[i] + a[i]
	}
	for i := 0; i < n+1; i++ {
		cost[i][i] = 0
	...
	for i := 1; i <= n; i++ {
		for l := 0; l+i <= n; l++ {
			...
			for k := 1; k < i; k++ {
				t := l + k
				chmin(&cost[l][r], cost[l][t]+cost[t][r]+weight[r]-weight[l])
			}
		}
	}

	out.Println(cost[0][n])
}
