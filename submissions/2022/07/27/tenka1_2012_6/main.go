...
func solve(s []byte) string {
	...
	toUpper := func(b byte) byte { return b - 'a' + 'A' }
	toLower := func(b byte) byte { return b - 'A' + 'a' }
	...
	for i, v := range s {
		...
		if v == '_' {
			...
			if i < len(s)-1 && ('z' < s[i+1] || s[i+1] < 'a') {
				isSnake = false
			}
		}
	}

	// debug(string(s), isSnake, isCamel)
	ans := []byte{}
...
func calc() {
	...
	out.Printf("%s%s%s\n", strings.Repeat("_", head), solve(s), strings.Repeat("_", tail))
}
