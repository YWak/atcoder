...
var mod = Mod1000000007()

func calc() {
	K := in.NextLongIntAsArray()
	d := in.NextInt()

	// dp[i][j][k] は i番目までみて各桁の総和のあまりがjで、現在の値がKの値を超えないことが確定してk(0:いない, 1:いる)ときの場合の数
	dp := make([][][]int, len(K)+1)
	for i := range dp {
		dp[i] = NewIntInt(d, 2, 0)
	}
	dp[0][0][0] = 1
	...
	for i := 0; i < len(K); i++ {
		for j1 := 0; j1 < d; j1++ {
			// k == 1ならどの値でも大丈夫。
			for j2 := 0; j2 < 10; j2++ {
				mod.chadd(&dp[i+1][(j1+j2)%d][1], dp[i][j1][1])
			}
			// k == 0で、0 <= j < K[i]となる値はk == 1になる
			for j2 := 0; j2 < K[i]; j2++ {
				mod.chadd(&dp[i+1][(j1+j2)%d][1], dp[i][j1][0])
			}
			// j == K[i]となる値はk == 0のまま
			mod.chadd(&dp[i+1][(j1+K[i])%d][0], dp[i][j1][0])
		}
	}

	ans := mod.add(dp[len(K)][0][0], dp[len(K)][0][1])
	mod.chsub(&ans, 1)
