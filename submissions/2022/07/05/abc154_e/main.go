...
// dp[i][j][k]は i桁目まで見て0以外の数値をj個使い、nより小さいことがk(0:不確定, 1:確定)したときの場合の数
var dp = [101][5][2]int{}
	...
	for i, n := range N {
		for j := 0; j <= k; j++ {
			...
			if n == 0 {
				// nが0のとき0を追加しても不確定のまま
				dp[i+1][j][0] += dp[i][j][0]
				// debug(i, n, fmt.Sprintf("dp[%d][%d][%d] += %d", i+1, j, 0, dp[i][j][0]))
			} else {
				// nが0より大きければ0を追加するとnより小さいことが確定する
				dp[i+1][j][1] += dp[i][j][0]
				// debug(i, n, fmt.Sprintf("dp[%d][%d][%d] += %d", i+1, j, 1, dp[i][j][0]))

				// 不確定のところにnを追加しても不確定のまま
				dp[i+1][j+1][0] += dp[i][j][0]
				// debug(i, n, fmt.Sprintf("dp[%d][%d][%d] += %d", i+1, j+1, 0, dp[i][j][0]))
			}

			// 確定している分については0を追加しても確定したまま
			dp[i+1][j][1] += dp[i][j][1]
			// debug(i, n, fmt.Sprintf("dp[%d][%d][%d] += %d", i+1, j, 1, dp[i][j][1]))

			// 1 <= d < n[i]となるdを追加した場合はnより小さい値が確定する
			if n > 0 {
				dp[i+1][j+1][1] += dp[i][j][0] * (n - 1)
				// debug(i, n, fmt.Sprintf("dp[%d][%d][%d] += %d", i+1, j+1, 1, dp[i][j][0]*(n-1)))
			}

			// 確定している分については1〜9までどの値を追加しても確定したまま
			dp[i+1][j+1][1] += dp[i][j][1] * 9
			// debug(i, n, fmt.Sprintf("dp[%d][%d][%d] += %d", i+1, j+1, 1, dp[i][j][1]*9))
		}
	}

	// 小さいことが確定している + 小さいことが確定していない == Nと同じ値
	out.Println(dp[len(N)][k][1] + dp[len(N)][k][0])
}
