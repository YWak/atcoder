...
// tested:
//   https://atcoder.jp/contests/abl/tasks/abl_d
func (st *SegmentTree) init(n int) *SegmentTree {
	// xはn*2を超える最小の2べき
	x := 1
	for x/2 < n+1 {
		x *= 2
...
// tested:
//   https://judge.yosupo.jp/problem/staticrmq
func (st *SegmentTree) initAsArray(vals []int) *SegmentTree {
	n := len(vals)
	// xはn*2を超える最小の2べき
	x := 1
	for x/2 < n {
...
func calc() {
	...
	small := NewSegmentTree().initAsArray(ss)
	large := NewSegmentTree().initAsArray(sl)

	ans := 0
	for i := 0; i < m; i++ {
		// iを開けることにしたときの最大スコア
		// debug(i, small.query(0, i), large.query(i+1, m))
		chmax(&ans, small.query(0, i)+large.query(i+1, m))
	}
	out.Println(ans)
}
