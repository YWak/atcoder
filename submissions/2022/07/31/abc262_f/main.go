...
// 先頭を全部消してから残りを辞書順にするパターン
func doOp1(p []int, k, first int) []int {
	ans := []int{first}
	rest := NewIntPriorityQueue(smaller)
	restref := map[int]int{}
	...
	// kが残っている分は消せる
	ans = append(ans, p[last+1:len(p)-k]...)

	return ans
}

// 後ろをgreedyにしてから前をgreedyにする
func doOp2(p []int, k, first int) []int {
	ans := []int{first}

	tail := NewIntPriorityQueue(smaller)
	tailref := map[int]int{}
	for i := len(p) - 1; i >= 0; i-- {
		k--
		if p[i] == first {
	...
	head := NewIntPriorityQueue(smaller)
	headref := map[int]int{}
	for i := 0; i < len(p); i++ {
		if p[i] == first {
			break
	...
	for k > 0 && !head.Empty() {
		t := head.Pop()
		r := headref[t]
		debug("op2", "front", t, r, last, r-last, k)
		if r > last && r-last <= k {
			k -= r - last
	...
	debug(front, last, k)
	front = append(front, p[last+1:headlen-k]...)

	if len(back) > 0 && len(front) > 0 && front[0] > back[0] {
		ans = append(ans, back...)
	}
...
func calc() {
	...
	ans := 0
	for i, l := 0, min(len(ans1), len(ans2)); i < l; i++ {
		if ans1[i] != ans2[i] {
			if ans1[i] < ans2[i] {
				ans = 1
			} else {
	...
	if ans == 1 {
		out.PrintIntsLn(ans1)
	} else {
		out.PrintIntsLn(ans2)
	}
}
