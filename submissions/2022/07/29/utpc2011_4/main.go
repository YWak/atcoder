...
func calc() {
	...
	for i := 0; i < R; i++ {
		...
		used[i] = make([][][]bool, C)
		for j := 0; j < C; j++ {
			used[i][j] = make([][]bool, 16)
			for k := 0; k < 16; k++ {
				used[i][j][k] = make([]bool, 4)
			}
		}
	}

	isDone := func(s *state) bool {
		return used[s.r][s.c][s.mem][s.dir]
	}
	done := func(s *state) {
		used[s.r][s.c][s.mem][s.dir] = true
	}

	ok := false
	r := func(dir int) int { return (dir + R) % R }
	c := func(dir int) int { return (dir + C) % C }
	m := func(mem int) int { return (mem + 16) % 16 }
	queue := []*state{{0, 0, 0, 0}}
	...
	for len(queue) > 0 {
		...
		switch o {
		case '<':
			next = append(next, &state{r(s.r + 0), c(s.c - 1), s.mem, 2})
		case '>':
			next = append(next, &state{r(s.r + 0), c(s.c + 1), s.mem, 0})
		case '^':
			next = append(next, &state{r(s.r - 1), c(s.c + 0), s.mem, 3})
		case 'v':
			next = append(next, &state{r(s.r + 1), c(s.c + 0), s.mem, 1})
		case '_':
			if s.mem == 0 {
				next = append(next, &state{r(s.r + 0), c(s.c + 1), s.mem, 0})
			} else {
		...
		case '?':
			...
			next = append(next, &state{r(s.r - 1), c(s.c + 0), s.mem, 3})
		case '.':
			next = append(next, &state{r(s.r + dr[s.dir]), c(s.c + dc[s.dir]), s.mem, s.dir})
		case '@':
			ok = true
			break
		case '+':
			next = append(next, &state{r(s.r + dr[s.dir]), c(s.c + dc[s.dir]), m(s.mem + 1), s.dir})
		case '-':
			next = append(next, &state{r(s.r + dr[s.dir]), c(s.c + dc[s.dir]), m(s.mem - 1), s.dir})
		default:
			next = append(next, &state{r(s.r + dr[s.dir]), c(s.c + dc[s.dir]), int(o - '0'), s.dir})
		}
		for _, v := range next {
			if !isDone(v) {
				done(v)
