...
func (t *Treap) Count(key interface{}) interface{} {
	n := t.root
	ans := 0
	for n != nil {
		...
		if c == 0 {
			ans += t._count(n) - t._count(n.right)
			break
		}
		if c > 0 {
			n = n.left
		} else {
			ans += t._count(n.left) + 1
			n = n.right
		}
	}
...
func calc() {
	...
	ans := 0
	for i := 0; i < n; i++ {
		p, q := st.query(x[i]+1, n+1), m[c[i]].Count(x[i]+1).(int)
		ans += p - q
		// debug(x[i], c[i], p, q)
		st.update(x[i], st.get(x[i])+1)
		t := m[c[i]].Get(x[i])
		if t == nil {
			t = 0
		}
		m[c[i]].Put(x[i], t.(int)+1)
	}
