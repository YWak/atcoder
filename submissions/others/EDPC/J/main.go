...
func main() {
	...
	memo = make([][][]float64, L)
	for i := 0; i < L; i++ {
		memo[i] = make([][]float64, L)
		for j := 0; j < L; j++ {
			memo[i][j] = make([]float64, L)
	...
	fmt.Println(solve(N, a[1], a[2], a[3]))
}

func solve(n, a1, a2, a3 int) float64 {
	if memo[a1][a2][a3] != -1.0 {
		return memo[a1][a2][a3]
	}
	a0 := n - a1 - a2 - a3
	f0 := float64(n - a0)
	d := float64(n) / f0
	...
	if a1 > 0 {
		d += solve(n, a1-1, a2, a3) * float64(a1) / f0
	}
	if a2 > 0 {
		d += solve(n, a1+1, a2-1, a3) * float64(a2) / f0
	}
	if a3 > 0 {
		d += solve(n, a1, a2+1, a3-1) * float64(a3) / f0
	}
	memo[a1][a2][a3] = d
	return d
}
