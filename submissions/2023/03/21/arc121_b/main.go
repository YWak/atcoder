...
func solve() int {
	...
	if len(c[0])%2 == 0 && len(c[1])%2 == 0 && len(c[2])%2 == 0 {
		return 0
	}
	...
	// l1とl2の中で絶対値が最小になる組合せの上位2つを探す。
	find := func(l1, l2 []int) (ans1, ans2, i1, i2 int) {
		ans1 = INF18
		ans2 = INF18
		i1, i2 = 0, 0

		u, v := 0, 0
		for u < len(l1) && v < len(l2) {
			for du := -1; du <= +1; du++ {
				for dv := -1; dv <= +1; dv++ {
					uu, vv := u+du, v+dv

					if uu < 0 || uu == len(l1) || vv < 0 || vv == len(l2) {
						continue
					}
	...
	// パターン1: c[1]とc[2]の一匹ずつだけをペアにする
	ans, _, _, _ := find(c[1], c[2])

	// パターン2: c[0]から2匹、c[1]とc[2]の1匹ずつをそれぞれペアにする
	if len(c[0]) >= 2 {
		a11, a12, i11, i12 := find(c[0], c[1])
		a21, a22, i21, i22 := find(c[0], c[2])
