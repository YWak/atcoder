...
func calc() {
	...
	// dp1[i][j]はi番目の料理まで見て時刻jのときの美味しさの合計の最大値
	// dp2[i][j]は↑のときのあまっている美味しさ
	dp0 := NewIntInt(n+1, t, 0)
	dp1 := NewIntInt(n+1, t, 0)
	dp2 := NewIntInt(n+1, t, 0)
	dp0[0][0] = 1
	for i := 0; i < n; i++ {
		for j := 0; j < t; j++ {
			// このケースが起こっていなければ無視する
			...
			// 食べない場合
			left := max(dp2[i][j], b[i])
			before, after := dp1[i+1][j]+dp2[i+1][j], dp1[i][j]+left
			if before < after || (before == after && dp2[i+1][j] < left) {
				dp0[i+1][j] = 1
				dp1[i+1][j], dp2[i+1][j] = dp1[i][j], left
			}

			// 食べる場合
			ii, jj := i+1, j+a[i]
			if jj < t {
				before, after = dp1[ii][jj]+dp2[ii][jj], dp1[i][j]+b[i]+dp2[i][j]
				if before < after || (before == after && dp2[ii][jj] < dp2[i][j]) {
					dp0[ii][jj] = 1
					dp1[ii][jj], dp2[ii][jj] = dp1[i][j]+b[i], dp2[i][j]
				}
			}
		}
	}
