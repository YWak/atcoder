...
func calc() {
	a := NewBigDecimal(in.NextInt())
	x := NewBigDecimal(in.NextInt())
	m := NewBigDecimal(in.NextInt())

	var ans int
	if x.EqualTo(Big.ZERO()) {
		ans = 1 % m.Int()
	} else if a.EqualTo(Big.ONE()) {
		ans = x.Mod(m).Int()
	} else {
		am := a.Sub(Big.ONE()).Mul(m)
		p := a.Pow(x, am).Sub(Big.ONE()).Div(a.Sub(Big.ONE()))
		ans = p.Mod(m).Int()
	}
