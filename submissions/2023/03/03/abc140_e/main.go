...
func calc() {
	...
	st := NewRangeMaxQuery()
	st.initAsArray(ps)
	// [l, r)でtよりも大きい数字のインデックスのうち、最大の値を返す。なければ負の値を返す。
	findLeft := func(l, r, t int) int {
		// どこにもなければ-INF
		if l < 0 || r < 0 || l >= r || st.query(l, r) < t {
			return -INF18
		}
	...
	// [l, r)でtよりも大きい数字のインデックスのうち、最小の値を返す。なければINFを返す。
	findRight := func(l, r, t int) int {
		// どこにもなければINF
		if l > n || r > n || l >= r || st.query(l, r) < t {
			return INF18
		}
	...
	for i, v := range ps {
		li := findLeft(0, i, v)
		lli := findLeft(0, li, v)
		ri := findRight(i+1, n, v)
		rri := findRight(ri+1, n, v)

		// debug(v, "i", lli, li, ri, rri)
		pat := 0
		// 最大値としてps[li]を使用するケース
		if li >= 0 {
			l := li - max(0, lli+1) + 1
			r := min(ri, n) - (i + 1) + 1
			pat += l * r
