...
func solve(s1, s2 []*sw, k int) float64 {
	// 濃度がok以上の砂糖水がk個以上作れるか
	check := func(d float64) bool {
		ord := func(s *sw) float64 {
			return s.x*100 - d*(s.x+s.y)
		}
		sort.Slice(s2, func(i, j int) bool {
			return ord(s2[i]) > ord(s2[j])
		})
