...
func calc() {
	...
	head := make([]bool, len(s)+1)
	tail := make([]bool, len(s)+1)
	...
	for i, b := range s {
		...
		// debug(fmt.Sprintf("%c, %c", t[i], b))
		head[i+1] = head[i] && same(t[i], b)
	}
	// debug("----")
	tail[0] = true
	for i := 0; i < len(t); i++ {
		// debug(fmt.Sprintf("%c, %c", t[len(t)-1-i], s[len(s)-1-i]))
		tail[i+1] = tail[i] && same(t[len(t)-1-i], s[len(s)-1-i])
	}

	for i := 0; i <= len(t); i++ {
		out.YesNo(head[i] && tail[len(t)-i])
	}
}
