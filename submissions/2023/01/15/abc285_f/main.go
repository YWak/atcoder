...
type SegmentTreeFunctions1 struct {
	// 単位元を返します
	e func() int
	// 計算結果を返します
	calc func(a, b int) int
...
//
//	https://judge.yosupo.jp/problem/staticrmq
func NewRangeMinQuery() *SegmentTree1 {
	return &SegmentTree1{
		-1,
		[]int{},
		SegmentTreeFunctions1{
...
//
//	https://judge.yosupo.jp/problem/staticrmq
func (st *SegmentTree1) initAsArray(vals []int) {
	n := len(vals)
	// xはn*2を超える最小の2べき
	x := 1
	for x/2 < n {
...
//
//	https://atcoder.jp/contests/abl/tasks/abl_d
func (st *SegmentTree1) update(i, value int) {
	t := i + st.n
	st.nodes[t] = value
...
//
//	https://atcoder.jp/contests/abl/tasks/abl_d
func (st *SegmentTree1) query(l, r int) int {
	ret := st.f.e()
	for ll, rr := l+st.n, r+st.n; ll < rr; ll, rr = ll/2, rr/2 {
		if ll%2 == 1 {
			ret = st.f.calc(ret, st.nodes[ll])
...
func calc() {
	...
	diff := make([]int, n)
	for i := 0; i < n-1; i++ {
		diff[i] = int(s[i+1]) - int(s[i])
	}

	st := NewSegmentTree()
	st.initAsArray(sv)
	rangemin := NewRangeMinQuery()
	rangemin.initAsArray(diff)

	for q := 0; q < Q; q++ {
		t := in.NextInt()
		if t == 1 {
			x := in.NextInt() - 1
			c := byte(in.NextString()[0]) - 'a'
			a := make([]int, 26)
			a[c]++
			st.update(x, a)
			...
			tc[c]++
			if x > 0 {
				rangemin.update(x-1, int(c)-int(s[x-1]))
				diff[x-1] = int(c) - int(s[x-1])
			}
			if x < n-1 {
				rangemin.update(x, int(s[x+1])-int(c))
				diff[x] = int(s[x+1]) - int(c)
			}
			s[x] = c
		} else {
			l := in.NextInt() - 1
			...
			sc := st.query(l, r)
			// 昇順ならOK
			ok := rangemin.query(l, r-1) >= 0
			// debug(diff)
			// debug(l, r, rangemin.query(l, r-1), tc, sc)
			// 始めの値が足りていればOK
			first := INF18
			last := -1
			for i, v := range sc {
