...
func calc() {
	...
	rangemin.initAsArray(a)
	rangemax := NewRangeMaxQuery()
	rangemax.initAsArray(a)
	...
	for q := 0; q < Q; q++ {
		...
		if t == 1 {
			k, x := in.NextInt2()
			k--
			rangemin.update(k, rangemin.get(k)+x)
			rangemax.update(k, rangemax.get(k)+x)
		} else {
			l, r := in.NextInt2()
			l--
			out.YesNo(rangemin.query(l, r) == rangemax.query(l, r))
		}
	}
}
