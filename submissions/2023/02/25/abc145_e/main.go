...
func calc() {
	...
	// dp[i][j]はi番目の料理まで見て、j分後までに食べ終えたときの満足度の最大値
	// re[i][j]はdp[i][j]を達成するときに注文しなかった料理のうち最も美味しい料理の美味しさ
	dp := NewIntInt(n+1, t+1, 0)
	re := NewIntInt(n+1, t+1, 0)
	update := func(i1, j1, i2, j2 int) bool {
		s1, s2 := dp[i1][j1]+re[i1][j1], dp[i2][j2]+re[i2][j2]
		if s1 != s2 {
			return s1 <= s2
		}
		return dp[i1][j1] <= dp[i2][j2]
	}

	for i := range a {
		for j := 0; j <= t; j++ {
			if j > 0 && update(i+1, j, i+1, j-1) {
				dp[i+1][j] = dp[i+1][j-1]
				re[i+1][j] = re[i+1][j-1]
			}
			// 何もしない場合
			if update(i+1, j, i, j) {
				dp[i+1][j] = dp[i][j]
				re[i+1][j] = max(re[i][j], b[i])
			}
			// 食べる場合
			jj := j + a[i]
			if jj <= t && update(i+1, jj, i, j) {
				dp[i+1][jj] = dp[i][j] + b[i]
				re[i+1][jj] = re[i][j]
			}
		}
	}
	out.Println(dp[n][t] + re[n][t])
}
