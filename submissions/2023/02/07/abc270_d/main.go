...
// memo[i]はi個の石が残っているときに取得できる最大数
var memo = [10001]int{0}
var done = [10001]bool{true}

func dp(curr int, a []int) int {
	if !done[curr] {
		done[curr] = true
		// 可能な全パターンで一番相手の取得する石が少なくなるパターンを選ぶ
		ans := INF18
		for _, v := range a {
			if v > curr {
				break
			}
			chmin(&ans, dp(curr-v, a))
		}
		memo[curr] = curr - ans
	}
