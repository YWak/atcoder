...
func dfs(g [][]int, curr, prev, color int) [2]map[int]bool {
	cs := [2]map[int]bool{{}, {}}
	...
	for _, next := range g[curr] {
		if done[next] || next == prev {
			continue
		}
		cs1 := dfs(g, next, curr, 1-color)
...
func calc() {
	...
	for p := range g {
		...
		for _, c := range cs {
			for u := range c {
				ans -= len(c) - 1 // 同じ色に設定されているノードは辺を張れない
				for _, v := range g[u] {
					if c[v] {
						// 同じ色どうしに辺がはられているなら二部グラフではなくなる
						ans = 0
					} else {
						// 既に辺がはられている分は答えから削る
						ans--
