...
func calc() {
	...
	for i := 0; i <= n; i++ {
		...
		hb_s, hb_e := h, h+i
		ta_s, ta_e := i+n, n*2
		tb_s, tb_e := n*2+j+i, n*3+j
		// debug(
		// 	fmt.Sprintf(
		// 		"ha[%d:%d]=%s, hb[%d,%d]=%s, ta[%d:%d]=%s, tb[%d:%d]=%s (%d, %d) (%d, %d)",
		// 		ha_s, ha_e, string(t2[ha_s:ha_e]),
		// 		hb_s, hb_e, string(t2[hb_s:hb_e]),
		// 		ta_s, ta_e, string(t2[ta_s:ta_e]),
		// 		tb_s, tb_e, string(t2[tb_s:tb_e]),
		// 		rh.Hash(ha_s, ha_e), rh.Hash(hb_s, hb_e),
		// 		rh.Hash(ta_s, ta_e), rh.Hash(tb_s, tb_e),
		// 	),
		// )
		if rh.Hash(ha_s, ha_e) != rh.Hash(hb_s, hb_e) {
			continue
		}
		if rh.Hash(ta_s, ta_e) != rh.Hash(tb_s, tb_e) {
			continue
		}
		ansi = i
		anss = append(anss, t[:i]...)
		anss = append(anss, t[i+n:n*2]...)
		break
	}
