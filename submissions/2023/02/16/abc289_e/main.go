...
func solve() {
	...
	for i := 0; i < m; i++ {
		...
		g[u][c[v]] = append(g[u][c[v]], v)
		g[v][c[u]] = append(g[v][c[u]], u)
	}

	// dp[i][j] は 高橋くんが頂点iに、青木くんが頂点jにいるときの最短ステップ
	dp := NewIntInt(n, n, INF18)
	dp[0][n-1] = 0
	type state struct{ ta, ao, c int }
	queue := []*state{{0, n - 1, 0}}
	for len(queue) > 0 {
		s := queue[0]
		queue = queue[1:]
		...
		for ta := 0; ta < 2; ta++ {
			ao := 1 - ta
			for _, u := range g[s.ta][ta] {
				for _, v := range g[s.ao][ao] {
					if dp[u][v] > s.c+1 {
						queue = append(queue, &state{u, v, s.c + 1})
						dp[u][v] = s.c + 1
					}
				}
			}
