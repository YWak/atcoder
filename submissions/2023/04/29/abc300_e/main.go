...
func solve(n int) int {
	...
	memo := make([][][]int, es[0]+1)
	for i := range memo {
		memo[i] = make([][]int, es[1]+1)
		for j := range memo[i] {
			memo[i][j] = make([]int, es[2]+1)
	...
	inv5 := mod.inv(5)
	var dp func(a, b, c int) int
	dp = func(c2, c3, c5 int) int {
		if c2 < 0 || c3 < 0 || c5 < 0 {
			return 0
		}
		...
		if memo[c2][c3][c5] == -1 {
			a := 0
			mod.chadd(&a, dp(c2-1, c3, c5))
			mod.chadd(&a, dp(c2, c3-1, c5))
			mod.chadd(&a, dp(c2-2, c3, c5))
			mod.chadd(&a, dp(c2, c3, c5-1))
			mod.chadd(&a, dp(c2-1, c3-1, c5))
			memo[c2][c3][c5] = mod.mul(a, inv5)
		}

		return memo[c2][c3][c5]
	}

	return dp(es[0], es[1], es[2])
}
