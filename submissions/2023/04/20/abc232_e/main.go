...
func calc() {
	...
	for t := 0; t < k; t++ {
		...
		// x軸について移動する場合
		for y := 0; y < 2; y++ {
			mod.chadd(&nx[0][y], mod.mul(dp[0][y], h2)) // 合っていないところから合っていないところへ
			mod.chadd(&nx[1][y], mod.mul(dp[0][y], 1))  // 合っていないところから合っているところへ
			mod.chadd(&nx[0][y], mod.mul(dp[1][y], h1)) // 合っているところから合っていないところへ
		}
		// y軸について移動する場合
		for x := 0; x < 2; x++ {
			mod.chadd(&nx[x][0], mod.mul(dp[x][0], w2))
			mod.chadd(&nx[x][1], mod.mul(dp[x][0], 1))
			mod.chadd(&nx[x][0], mod.mul(dp[x][1], w1))
		}
