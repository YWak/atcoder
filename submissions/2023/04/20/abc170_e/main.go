...
func calc() {
	...
	for i := 1; i <= n; i++ {
		rate[i], kinder[i] = in.NextInt2()
		add(ks[kinder[i]], rate[i])
	}
	st := NewRangeMinQuery()
	st.init(len(ks))
	for i, m := range ks {
	...
	for q := 0; q < Q; q++ {
		...
		sub(ks[from], rate[c])
		add(ks[to], rate[c])
		st.update(from, top(ks[from]))
		st.update(to, top(ks[to]))
		kinder[c] = to
		out.Println(st.all())
	}
}
