...
func dinic(g [][]*edge, edges []*edge, s, t int) int {
	...
	dfs = func(curr, f int) int {
		...
		for i := loopstart[curr]; i < len(g[curr]); i++ {
			loopstart[curr] = i
			e := g[curr][i]
...
func calc() {
	...
	for i, p := range ps {
		...
		e3 := &edge{T, N + i, l + 3, 1}
		e4 := &edge{N + i, T, l + 2, 0}
		edges = append(edges, e1, e2, e3, e4)
		g[e1.from] = append(g[e1.from], e1)
		g[e2.from] = append(g[e2.from], e2)
		g[e3.from] = append(g[e3.from], e3)
		g[e4.from] = append(g[e4.from], e4)
	}

	out.Println(dinic(g, edges, 0, T))
}
