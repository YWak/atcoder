...
func calc() {
	...
	// dp[i][j][k] m個使うとしたときにi番目の素材まで見て、j個の素材を使用して、魔力の合計(mod m)がkである場合の魔力の最大値
	ans := INF18
	for m := 1; m <= n; m++ {
		dp := make([][][]int, n+1)
		for i := range dp {
			dp[i] = NewIntInt(m+1, m, -1)
		}
		dp[0][0][0] = 0
...
func extmul(as, b int) (int, int) {
	if as < 0 {
		as, b = -as, -b
	}
	if as == 0 || b == 0 {
		return 0, 0
	} else if as > 0 && b > 0 && as > math.MaxInt64/b {
		return 0, +1
	} else if as > math.MinInt64/b {
		return 0, -1
	}
	return as * b, 0
}
...
func xor(as, b bool) bool { return as != b }
