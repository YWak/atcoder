...
func calc() {
	...
	g := make([][]int, len(index))
	for _, s1 := range rindex {
		for _, s2 := range rindex {
			d := 0
	...
	dist := make([]int, len(index))
	prev := make([]int, len(index))
	...
	if ans == INF18 {
		...
		out.Println(ans - 1)
		route := make([]string, ans+1)
		route[ans] = last
		i := ans - 1
		for now := prev[lastindex]; now != -1; now = prev[now] {
			route[i] = rindex[now]
			i--
		}
		for _, v := range route {
