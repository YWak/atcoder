...
func check(l int, g [][]*edge, m int) {
	...
	dp[0][0] = 1
	for i, es := range g {
		for k, v := range dp[i] { // 既存の値について
			for _, e := range es { // 各辺を通って辿れるところに個数を増やす
				dp[e.v][e.w+k] += v
			}
		}
	}
	...
	if len(no) != 0 {
		if len(no) > 5 {
			panic(fmt.Sprintf("%d of numbers are not generated", len(no)))
		} else {
			panic(fmt.Sprintf("%v is not generated once", no))
		}
	}
}
...
func calc() {
	...
	// 全部0でつなぐ
	for i := 0; i < n-1; i++ {
		g[i] = append(g[i], &edge{i + 1, 0})

		if i < big {
			g[i] = append(g[i], &edge{i + 1, 1 << i})
		}
	}

	// 必要な辺を作る
	// debug(l, fmt.Sprintf("%022b", l))
	for i := 0; i <= big; i++ {
		if nthbit(l, i) == 1 {
			// i-1ビット目以下を任意に設定できる
			p := (1 << n) - (1 << (i + 1))
			...
			// debug(fmt.Sprintf("%2d %5d %022b %022b", i, 1<<i, t, p))

			g[i] = append(g[i], &edge{n - 1, t})
		}
	}
