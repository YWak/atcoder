...
func calc() {
	...
	dist := make([]int, 0, len(dm))
	for d := range dm {
		dist = append(dist, d)
	}
	sort.Ints(dist)
	search := func(d int) int {
		lower, upper := -1, len(dist)
		for abs(lower-upper) > 1 {
			mid := (lower + upper) / 2
			if dist[mid] >= d {
				upper = mid
			} else {
	...
	// dist[i]以上の間隔は何行あるか？
	tailCount := make([]int, len(dist)+1)
	tailLines := make([]int, len(dist)+1)
	for i := len(dist) - 1; i >= 0; i-- {
		tailCount[i] = tailCount[i+1] + dm[dist[i]]
		tailLines[i] = tailLines[i+1] + dm[dist[i]]*dist[i]
	}
	...
	for i := 0; i < m; i++ {
		...
		r := search(xy)
		if r != -1 {
			ans -= tailLines[r] - tailCount[r]*(xy)
		}
