	...
	for queue.Len() > 0 {
		...
		for t := 0; t < 4; t++ {
			...
			if cost[x][y] > nc {
				cost[x][y] = nc
				if c == 0 {
					queue.PushFront(&Point{x, y, nc})
				} else {
					queue.PushBack(&Point{x, y, nc})
				}
			}
		}
	}

	out.YESNO(cost[gx][gy] <= 2)
}
...
func (deque *Deque) PushFront(value *Point) {
	if deque.reversed {
...
func (deque *Deque) PushBack(value *Point) {
	if deque.reversed {
		deque.pushFrontInternal(value)
	} else {
		deque.pushBackInternal(value)
...
func (deque *Deque) PopFront() *Point {
	if deque.reversed {
...
func (deque *Deque) PopBack() *Point {
	if deque.reversed {
		return deque.popFrontInternal()
	}
	return deque.popBackInternal()
...
func (deque *Deque) pushBackInternal(value *Point) {
...
func (deque *Deque) pushFrontInternal(value *Point) {
...
func (deque *Deque) popFrontInternal() *Point {
	node := deque.head
...
func (deque *Deque) popBackInternal() *Point {
	node := deque.tail
