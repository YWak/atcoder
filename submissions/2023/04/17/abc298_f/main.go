...
func calc() {
	...
	sort.Slice(topr, func(i, j int) bool { return rs[topr[i]] > rs[topr[j]] })
	sort.Slice(topc, func(i, j int) bool { return cs[topc[i]] > cs[topc[j]] })

	ans := 0
	for _, r := range topr {
		for i := 0; i < min(2, len(topc)); i++ {
			c := topc[i]
			chmax(&ans, rs[r]+cs[c]-rc[point{r, c}])
		}
	}
	for _, c := range topc {
		for i := 0; i < min(2, len(topr)); i++ {
			r := topr[i]
			chmax(&ans, rs[r]+cs[c]-rc[point{r, c}])
		}
	}
