...
type UnionFind struct {
	par []int32 // i番目のノードに対応する親
}

// [0, n)のノードを持つUnion-Findを作る
func ufNew(n int) UnionFind {
	uf := UnionFind{par: make([]int32, n)}
...
// Root はxのルートを得る
func (uf *UnionFind) Root(x int32) int32 {
	if uf.par[x] < 0 {
		return x
	}
...
func (uf *UnionFind) Unite(x, y int32) bool {
	rx := uf.Root(x)
	ry := uf.Root(y)
...
func (uf *UnionFind) Size(x int32) int {
	return int(-uf.par[uf.Root(x)])
}
