...
func main() {
	...
	NN := len(po) * len(ne)
	NZ := NN + (len(po)+len(ne))*zero + zero*(zero-1)/2

	ans := 0

	sort.Sort(sort.IntSlice(po))
	sort.Sort(sort.IntSlice(ne))
	if K <= NN {
		// ok は ok 以下の数がK個以上作れる最小の数
		ng, ok := -INF, 0
		for abs(ok-ng) > 1 {
			mid := (ok + ng) / 2
			c := 0
			j := 0
			for i := len(ne) - 1; i >= 0; i-- {
				for {
					if j == len(po) || ne[i]*po[j] >= -mid {
						break
					}
					j++
				}
		...
		ng, ok := 0, INF // ok は ok 以下の数がKこ作れる最小の数
		count := func(n int, a []int) int {
			c := 0
			j := len(a)
			for i := 0; i < len(a); i++ {
				for {
				...
				// j は ai * aj >= nが成立する最小のaをポイントしている
				c += j
				// debug(n, c, i, j, a)
				if j != 0 && j > i { // a[i]が範囲に含まれる場合は一個減らす
					c--
				}
			}
			return c
