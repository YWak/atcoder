...
func main() {
	N, X, M := nextInt3()
	a := X
	histi := map[int]int{X: 1} // histi[x]は xが初めて現れたときのnの値
	hists := make([]int, M+1)  // hists[i]は i番目までの合計
	hists[1] = a
	f := func(a int) int { return (a * a) % M }
	for i := 2; i <= N; i++ {
		a = f(a)
		_, ex := histi[a]
		if ex {
			//確定したら1周分の合計をできるだけ足して、続きを行う。
			j := histi[a]
			l := i - j           // 一周分の要素
			c := (N - j + 1) / l // ループの回数
			r := (N - j + 1) % l // 残りの要素数
			...
			s := hists[j-1]                    // これまでの合計
			s += (hists[i-1] - hists[j-1]) * c // ループで増える分
			s += (hists[j+r-1] - hists[j-1])   // ループの残り
			fmt.Println(s)
			return
		}
		hists[i] = hists[i-1] + a
