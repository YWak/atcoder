...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		a--
		b--
		graph[a] = append(graph[a], edge{b, t, k})
		graph[b] = append(graph[b], edge{a, t, k})
	}
	ans := Dijkstra(x, y)
	if ans == INF {
		fmt.Println(-1)
...
func Dijkstra(s, t int) int {
	...
	for pq.Len() > 0 {
		...
		for i := 0; i < len(graph[u.node]); i++ {
			v := graph[u.node][i]
			cc := cost[u.node]
			c := ch(cc%v.k == 0, cc, cc+v.k-cc%v.k) + v.t
